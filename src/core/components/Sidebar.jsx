...
export default function Sidebar2({ className = "", user, handleLogout }) {
  ...
  return (
    <div
      id="sidebar"
      className={`transition-all duration-200 p-4 relative bg-white rounded-xl ml-4 mt-4 mb-4 shadow-lg border border-gray-200  min-h-40 overflow-y-auto
    ...
    >
      ...
      <div className="mt-8 flex flex-col items-center">
        <h1
          className={`uppercase font-bold text-primary transition-all ${
            collapsed ? "text-2xl" : "text-6xl"
          }`}
        >
          HINT
        </h1>
      ...
      <div
        className={`text-white transition-all duration-300 ${
          isShortScreen ? "relative" : "absolute bottom-0 mb-5 left-12"
        }`}
      >
        <Link
          href="/dashboard/profile"
          className={`btn btn-ghost flex flex-row items-center gap-4 px-4 py-10 w-full ${
            collapsed
        ...
        <button
          ...
          className={`btn btn-outline btn-primary w-full mt-1 ${
            collapsed ? "px-2 py-2 min-w-[60px] max-w-[60px]" : " "
          }`}
        >
